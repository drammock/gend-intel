...
def parse_args():
    ...
    parser.add_argument('input_csv', type=str,
                        help='a CSV file (with headers) containing the scores for each subject')
...
def makeDicts(data):
	...
	with open(data) as f:
		...
		for line in reader:
			subj = line[1]
			snr = int(line[3])
			# The following indices assume that we did two rounds of scoring, relaxed and strict
...
def main():
	...
	for subj in subjDict:
		...
		plot(data, subj)
	# Plot all
	toplot = [allDict[-3], allDict[-2], allDict[-1], allDict[0], allDict[1], allDict[2]]
